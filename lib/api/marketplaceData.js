    ...
    app.route('/api/soldABTs')
        .get(async (req, res) => {
            ...
            try {
                ...
                const lastSaleDate = soldABTs.length > 0
                    ? new Date(Math.max(...soldABTs.map(sale => new Date(sale.timestamp))))
                    : null;
    ...
    app.route('/api/grossRevenue')
        .get(async (req, res) => {
            ...
            try {
                ...
                const grossRevenueLast24Hours = proceedsLast24Hours.reduce((acc, curr) => {
                ...
                const tenProceeds = proceeds.sort((a, b) => b.timestamp - a.timestamp).slice(0, 10);
                ...
                res.status(200).json({ grossRevenue, inflowPercentage, tenProceeds, histogram });
            } catch (error) {
