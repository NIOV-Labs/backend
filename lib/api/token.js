...
function addRoutes(app, evm) {
	...
	function saveTmpFile(fileName, tokenId, network) {
		...
		const networkTokenDir = path.join(
			uploadsDir,
			network.toString(),
			tokenId.toString()
		); // Modified to include network
		...
		const urlPrefix = 'https://beta.niovlabs.io/uploads/';
		const relativeFileName = fileName.replace(urlPrefix, '');
		const baseName = relativeFileName
			.replace(tmpPrefix, '')
			.replace(path.extname(relativeFileName), '');
		...
		return path.join(
			network.toString(),
			tokenId.toString(),
			`${baseName}${path.extname(relativeFileName)}`
		);
	}
