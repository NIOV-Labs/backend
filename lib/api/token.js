...
function addRoutes(app, evm) {
	...
	function saveTmpFile(fileName, tokenId, network) {
        ...
        const networkTokenDir = path.join(uploadsDir, network.toString(), tokenId.toString()); // Modified to include network
  ...
  app
    ...
    .post(async (req, res) => {
        ...
        try {
            ...
            metadata.images = metadata.images.map(image => saveTmpFile(image, tokenId, network));
    ...
    .get(async (req, res) => {
        ...
        try {
            const token = await Token.findOne({ onChainID: tokenId, chainID: chainId }); // Filter by chainId
            if (!token) {
