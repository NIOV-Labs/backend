...
const fs = require('fs');
const { uploadLabels, uploadPaths, decomposeFile, deleteFiles, convertPdfToJpeg } = require('./files.js');

function addRoutes(app) {
    app.use(express.json({ limit: '50mb' })); // Ensure the server can parse large JSON bodies
    ...
    app
        ...
        .post((req, res) => {
            ...
            try {
                const { ext, chunk, chunkIndex, totalChunks, baseName } = req.body;
                console.log(`Parsed request: ext=${ext}, chunkIndex=${chunkIndex}, totalChunks=${totalChunks}, baseName=${baseName}`);

                const dataChunk = decomposeFile(chunk, chunkIndex, totalChunks);
                console.log(`Processing Chunk ${dataChunk.num} of ${dataChunk.total} || ${dataChunk.percent}%`);
