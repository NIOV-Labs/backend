...
async function setupListeners(evm) {
	...
	networks.forEach((chainId) => {
		const provider = evm.network.websocket(chainId);
		provider._websocket.on('close', () => {
			console.error(`WebSocket connection closed for ${chainId}`);
			reconnect(evm);
		});
		provider._websocket.on('error', (err) => {
			console.error(`WebSocket error for ${chainId}`, err);
			reconnect(evm);
		});
		sockets[chainId] = provider;
	});
